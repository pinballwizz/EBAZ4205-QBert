...
//Multiplex cpu rom data
wire [7:0] rom_dout =
		(~B8_Y[7] & ~B4_Y[3]) ? C11_12_Q:
		(~B8_Y[6] & ~B4_Y[3]) ? C12_13_Q:
		(~B8_Y[5] & ~B4_Y[3]) ? C13_14_Q:
//		(~B8_Y[4] & ~B4_Y[3]) ? C14_15_Q:
//		(~B8_Y[3] & ~B4_Y[3]) ? C16_Q:
		8'b0;
...
wire [10:0] nvram_addr = addr[10:0]; //rom_init ? rom_init_address[10:0] : addr[10:0];
...
// next 2 prg roms not used in qbert
/*
PRG_ROM_3 C14_15(
.clk(clk_sys),
.addr(PRG_addr),
...
//--------------------------------------------------------------
reg [13:0] FGROM_addr;
always @(posedge clk_sys) FGROM_addr <= {1'b0,RA}; // OK fixed!
//--------------------------------------------------------------
// FG_ROMS
